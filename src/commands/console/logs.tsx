...
export function ServerLogs({ server, ws }: ServerConsoleProps) {
    ...
    useInput((key, input) => {
        ...
        // Scroll Offset
        if (input.upArrow)
            setScrollOffset((o) => Math.min(o + 1, Math.max(0, logs.length - visibleLines)));
    ...
    const visibleLogs = logs.slice(
        -(lines + scrollOffset),
        scrollOffset === 0 ? undefined : -scrollOffset,
    );

    const statusColor =
        (
            { running: "green", offline: "red", connecting: "yellow" } as Record<
                string,
                string | undefined
            >
        )[status] || "gray";
        ...
        <Box flexDirection="column" flexGrow={1} height={process.stdout.rows - 1}>
            ...
            >
                {scrollOffset > 0 && (
                    <Text color="yellow">
                        {" "}
                        ↑ scrolled ({scrollOffset} lines from bottom) — ^D to jump to end
                    </Text>
                )}
